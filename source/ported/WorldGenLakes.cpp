...
namespace javaport
{
    bool WorldGenLakes::generate(Random &rng, Vec3i pos)
    {
        ...
        while (pos.y > 0 && world->get_block_id_at(pos, BlockID::air) == BlockID::air)
            pos.y--;
        ...
        for (int x = 0; x < 16; x++)
        {
            for (int z = 0; z < 16; z++)
            {
                for (int y = 0; y < 8; y++)
                {
                    ...
                    if (flag)
                    {
                        BlockID id = world->get_block_id_at(pos + Vec3i(x, y, z), BlockID::air);
        ...
        for (int x = 0; x < 16; x++)
        {
            for (int z = 0; z < 16; z++)
            {
                for (int y = 0; y < 8; y++)
                {
                    ...
                    {
                        world->set_block_at(pos + Vec3i(x, y, z), y >= 4 ? BlockID::air : liquid);
                    }
                }
            }
        }
        ...
        for (int x = 0; x < 16; x++)
        {
            for (int z = 0; z < 16; z++)
            {
                for (int y = 4; y < 8; y++)
                {
                    if (placements[(x * 16 + z) * 8 + y] && world->get_block_id_at(pos + Vec3i(x, y - 1, z), BlockID::air) == BlockID::dirt)
                    {
                        world->set_block_at(pos + Vec3i(x, y - 1, z), BlockID::grass);
                    }
                }
            }
        }
        ...
        if (liquid == BlockID::lava)
        {
            for (int x = 0; x < 16; x++)
            {
                for (int z = 0; z < 16; z++)
                {
                    ...
                    {
                        bool flag = !placements[(x * 16 + z) * 8 + y] && ((x < 15 && placements[((x + 1) * 16 + z) * 8 + y]) || (x > 0 && placements[((x - 1) * 16 + z) * 8 + y]) || (z < 15 && placements[(x * 16 + (z + 1)) * 8 + y]) || (z > 0 && placements[(x * 16 + (z - 1)) * 8 + y]) || (y < 7 && placements[(x * 16 + z) * 8 + (y + 1)]) || (y > 0 && placements[(x * 16 + z) * 8 + (y - 1)]));
                        if (flag && (y < 4 || rng.nextInt(2) == 0) && world->get_block_id_at(pos + Vec3i(x, y, z), BlockID::air) == BlockID::air)
                        {
                            world->set_block_at(pos + Vec3i(x, y, z), BlockID::stone);
                        }
                    }
                }
            }
