...
Frame *
frame_push(Code_attribute *code, ClassFile *class, U2 max_locals, U2 max_stack)
{
	Frame *frame = NULL;
	Value *local = NULL;
	Value *stack = NULL;
	...
	if (frame == NULL || (max_locals && local == NULL) || (max_stack && stack == NULL)) {
		free(frame);
		free(local);
		free(stack);
		return NULL;
...
Heap *
array_new(int32_t *nmemb, U1 dimension, size_t size)
{
	Heap *h;
	int32_t i;
