...
#include <cstdint>
uint8_t p[512]{0};
const uint8_t permutation[256]{151, 160, 137, 91, 90, 15,
...
void ImprovedNoise::noise_set(vec3f &pos, vec3i &size, float frequency, float amplitude, uint8_t *out)
{
    float inv_frequency = 1.0f / frequency;
    float half_amplitude = amplitude * 0.5f;
    float _z = pos.z * inv_frequency;
    for (int i = 0; i < size.z; i++, _z += inv_frequency)
    {
        float _y = pos.y * inv_frequency;
        for (int j = 0; j < size.y; j++, _y += inv_frequency)
        {
            float _x = pos.x * inv_frequency;
            for (int k = 0; k < size.x; k++, _x += inv_frequency)
            {
                float x = _x;
                float y = _y;
                float z = _z;
                ...
                *(out++) = uint8_t(half_amplitude + (half_amplitude)*lerp(w, lerp(v, lerp(u, grad(p[AA], x, y, z),        // AND ADD
                                                                                          grad(p[BA], x - 1, y, z)),      // BLENDED
                                                                                  lerp(u, grad(p[AB], x, y - 1, z),       // RESULTS
                                                                                       grad(p[BB], x - 1, y - 1, z))),    // FROM  8
                                                                          lerp(v, lerp(u, grad(p[AA + 1], x, y, z - 1),   // CORNERS
